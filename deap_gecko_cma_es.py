...
def cma_es(individual, fitness_func, sigma=0.5, population_size=10, generations=20):
    """
    Run CMA-ES optimization for a given individual.

    Args:
        individual (list): Initial genome.
        fitness_func (callable): Function to evaluate fitness.
        sigma (float): Initial standard deviation.
        population_size (int): Number of individuals per generation.
        generations (int): Number of generations to run.

    Returns:
        best_individual (list): Best genome found.
        best_fitness (float): Fitness of the best genome.
    """
    import cma

    # Run CMA-ES
    es = cma.CMAEvolutionStrategy(individual, sigma, {'popsize': population_size})
    for _ in range(generations):
        solutions = es.ask()
        fitnesses = [fitness_func(sol) for sol in solutions]
        es.tell(solutions, fitnesses)
        es.logger.add()
        es.disp()
    
    # Get best solution found
    best_individual = es.result.xbest
